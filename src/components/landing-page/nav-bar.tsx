...
const NavBar = ({ onSignIn, onSignUp }: NavbarProps) => {
  ...
  return (
    <nav className={`fixed top-0 inset-x-0  px-6 transition-all duration-300 ${scrolled?'bg-[#0b2618f5] border-b backdrop-filter-[16px]':'transparent border-b-none backdrop-filter-none'}`}>
      ...
      {open && (
        ...
        >
           <div className="flex gap-2.5 mt-3">
            <Button variant="outline" onClick={onSignIn} className="flex-1" style={{ background: "transparent", color: "white", borderColor: "rgba(255,255,255,0.25)", borderRadius: 14, height: "auto", padding: "12px" }}>Sign In</Button>
            <Button onClick={onSignUp} className="flex-1" style={{ background: "var(--shamba-emerald)", color: "var(--shamba-forest)", borderRadius: 14, height: "auto", padding: "12px", fontWeight: 700 }}>Get Started</Button>
          </div>
        </div>
      )}
    </nav>
