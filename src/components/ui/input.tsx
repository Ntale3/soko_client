...
const LeftIconContainer=({children,isFocused}:{children:ReactNode,isFocused:boolean})=>(
  <div className={cn("flex h-full items-center justify-center border-r border-r-muted px-3 transition-colors",{"border-r-primary":isFocused})}>
    <div className="flex items-center gap-2">{children}</div>
  </div>

)

const RightIconContainer =({children,isFocused,onClick}:{children:ReactNode,isFocused:boolean,onClick?:()=>void})=>(
  <div
  onClick={onClick}
  className={cn("flex h-full items-center justify-center border-l border-l-muted px-3 transition-colors",
...
const resolvedLeftIcon = isTel
  ...
  return (
    ...
    >
      ...
      {
        resolvedLeftIcon && (
          <LeftIconContainer isFocused={isFocused}>
            {resolvedLeftIcon}
          </LeftIconContainer>
        )
      }
      ...
      {/* Right icon */}
      {resolvedRightIcon && (
        <RightIconContainer isFocused={isFocused}>
          {resolvedRightIcon}
        </RightIconContainer>
      )}
